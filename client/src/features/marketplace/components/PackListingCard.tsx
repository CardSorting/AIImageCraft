  ...
  return (
    <Card 
      className={cn(
        ...
        layout === 'grid' ? "w-full h-full" : "w-full",
          ...
          <CardHeader className="p-4">
            ...
            <div>
              <div className="flex items-center justify-between gap-2">
                <h3 className="text-base font-semibold truncate">{listing.pack.name}</h3>
                <span className={cn(
                  "px-2 py-0.5 rounded-full text-xs font-medium",
                  listing.status === 'ACTIVE' && "bg-green-500/10 text-green-500",
                  listing.status === 'SOLD' && "bg-green-500/10 text-green-500",
                  listing.status === 'CANCELLED' && "bg-red-500/10 text-red-500"
                )}>
